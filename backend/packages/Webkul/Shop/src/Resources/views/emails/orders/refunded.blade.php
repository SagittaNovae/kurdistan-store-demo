@component('shop::emails.layout')
    ...
    @slot('preheader')@lang('shop::app.emails.orders.refunded.preheader')@endslot

    {{-- ─── Hero ─────────────────────────────────────────────────────────────── --}}
    <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin-bottom: 36px;">
        <tr>
            <td>
                <span style="display: inline-block; padding: 3px 10px; background-color: #0A1A1F; border: 1px solid #1A2E3A; border-radius: 4px; font-family: 'Inter', sans-serif; font-size: 10px; font-weight: 600; letter-spacing: 1.5px; text-transform: uppercase; color: #38BDF8;">
        ...
        <tr>
            <td style="padding-top: 12px;">
                <p style="margin: 0; font-family: 'Inter', sans-serif; font-size: 15px; color: #888888; line-height: 26px;">
                    @lang('shop::app.emails.dear', ['customer_name' => $refund->order->customer_full_name]) — {!! trans('shop::app.emails.orders.refunded.greeting', [
                        'invoice_id' => $refund->increment_id,
                        'order_id'   => '<a href="' . route('shop.customers.account.orders.view', $refund->order_id) . '" style="color: #FFFFFF; font-weight: 600; text-decoration: none;">#' . $refund->order->increment_id . '</a>',
                        'created_at' => core()->formatDate($refund->order->created_at, 'Y-m-d'),
                    ]) !!}
                </p>
            </td>
        </tr>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin-bottom: 44px;">
        <tr>
            <td>
                <a href="{{ route('shop.customers.account.orders.view', $refund->order_id) }}"
                    ...
                    <tr>
                        <td style="padding: 20px;">
                            ...
                            <p style="margin: 0 0 4px; font-family: 'Inter', sans-serif; font-size: 14px; color: #FFFFFF; font-weight: 500;">
                                @if ($refund->order->shipping_address->company_name){{ $refund->order->shipping_address->company_name }}<br/>@endif
                                {{ $refund->order->shipping_address->name }}
                            </p>
                            ...
                            @if ($refund->order->billing_address && $refund->order->billing_address->phone)
                            <p style="margin: 6px 0 0; font-family: 'Inter', sans-serif; font-size: 13px; color: #666666;">{{ $refund->order->billing_address->phone }}</p>
                            @endif
                        </td>
                    </tr>
                </table>
                ...
                <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="border: 1px solid #1A1A1A; border-radius: 10px;">
                    <tr>
                        <td style="padding: 20px;">
                            ...
                            <p style="margin: 0 0 4px; font-family: 'Inter', sans-serif; font-size: 14px; color: #FFFFFF; font-weight: 500;">
                                @if ($refund->order->billing_address->company_name){{ $refund->order->billing_address->company_name }}<br/>@endif
                                {{ $refund->order->billing_address->name }}
                            </p>
                            <p style="margin: 0 0 14px; font-family: 'Inter', sans-serif; font-size: 13px; color: #888888; line-height: 22px;">
                                ...
                                {{ $refund->order->billing_address->city }}, {{ $refund->order->billing_address->state }}<br/>
                                {{ $refund->order->billing_address->country }}
                            </p>
    ...
    {{-- ─── Refunded items ─────────────────────────────────────────────────────── --}}
    <p style="margin: 0 0 16px; font-family: 'Inter', sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; color: #555555;">Refunded Items</p>
                ...
                <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                    <tr>
                        ...
                        <td valign="top" style="vertical-align: top;">
                            ...
                            <p style="margin: 0; font-family: 'Inter', sans-serif; font-size: 12px; color: #555555;">@lang('shop::app.emails.orders.qty'): {{ $item->qty }}</p>
                        ...
                        <td valign="top" align="right" style="padding-left: 16px; white-space: nowrap; vertical-align: top;">
                            <p style="margin: 0; font-family: 'Inter', sans-serif; font-size: 14px; font-weight: 600; color: #38BDF8;">
                                @if (core()->getConfigData('sales.taxes.sales.display_prices') == 'including_tax')
                                    {{ core()->formatPrice($item->total_incl_tax, $refund->order_currency_code) }}
                                @elseif (core()->getConfigData('sales.taxes.sales.display_prices') == 'both')
                                    {{ core()->formatPrice($item->total_incl_tax, $refund->order_currency_code) }}
                                @else
                                    {{ core()->formatPrice($item->total, $refund->order_currency_code) }}
                                @endif
                            </p>
                        </td>
                    </tr>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin-top: 28px;">
        ...
        <tr>
            ...
            <td align="right" style="padding-bottom: 10px; font-family: 'Inter', sans-serif; font-size: 14px; color: #FFFFFF;">{{ core()->formatPrice($refund->sub_total_incl_tax, $refund->order_currency_code) }}</td>
        </tr>
        ...
        <tr>
            ...
            <td align="right" style="padding-bottom: 10px; font-family: 'Inter', sans-serif; font-size: 14px; color: #FFFFFF;">{{ core()->formatPrice($refund->sub_total, $refund->order_currency_code) }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td align="right" style="padding-bottom: 10px; font-family: 'Inter', sans-serif; font-size: 14px; color: #FFFFFF;">{{ core()->formatPrice($refund->shipping_amount, $refund->order_currency_code) }}</td>
        </tr>
        @endif
        ...
        <tr>
            ...
            <td align="right" style="padding-bottom: 10px; font-family: 'Inter', sans-serif; font-size: 14px; color: #FFFFFF;">{{ core()->formatPrice($refund->tax_amount, $refund->order_currency_code) }}</td>
        </tr>
        @endif
        ...
        <tr>
            ...
            <td align="right" style="padding-bottom: 10px; font-family: 'Inter', sans-serif; font-size: 14px; color: #4ADE80;">-{{ core()->formatPrice($refund->discount_amount, $refund->order_currency_code) }}</td>
        </tr>
        @endif
        ...
        <tr>
            ...
            <td align="right" style="padding-top: 12px; font-family: 'Inter', sans-serif; font-size: 18px; font-weight: 700; color: #38BDF8;">{{ core()->formatPrice($refund->grand_total, $refund->order_currency_code) }}</td>
        </tr>
    </table>
